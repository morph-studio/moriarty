...
class SpawnerManager(metaclass=Singleton):
    def __init__(self):
        self.pm = pluggy.PluginManager(plugin.project_name)
        self.pm.add_hookspecs(plugin)
        self._registed_cls: dict[str, type[plugin.Spawner]] = {}
    ...
    def registed_cls(self) -> dict[str, type]:
        ...
        for f in self.pm.hook.register(manager=self):
            try:
                ...
                logger.error(f"One spawner registration failed: {e}")
                logger.exception(e)
                continue
        return self._registed_cls
    ...
    def register(self, cls: type[plugin.Spawner]):
        """
    ...
    def init(self, c, *args, **kwargs: dict[str, Any]):
        ...
        """
        if isinstance(c, plugin.Spawner):
            return c
